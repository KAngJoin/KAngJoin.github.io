...
- global.component.config.js（获取高频组件，处理并全局注册）

```javascript
//获取高频组件
const requireComponent = require.context('./', false, /\.vue$/)
// 首字母大写，vue组件命名都是首字母大写的。
...
const install = (Vue) => {
  ...
  requireComponent.keys().forEach(key => {
    let vueTemplate = requireComponent(key);
    // 获取到高频组件的名词
    // key.substring(key.lastIndexOf('/') + 1).replace(/\.\w+$/, '') 可嵌套的
    let componentName = changeStr(key.substring(key.lastIndexOf('/') + 1).replace(/\.\w+$/, ''))
    Vue.component(componentName, vueTemplate.default || vueTemplate)
  });
}
...
//全局注册
Vue.use(GlobalTemplate) //// 调用 `GlobalTemplate.install(Vue)`
// ...
```
...
>
> [vuex]状态字段“Home”被一个在“Home”中具有相同名称的模块覆盖
