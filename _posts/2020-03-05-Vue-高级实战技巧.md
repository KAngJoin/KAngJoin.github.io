...
<script>
import { store } from "../../store.oneself.js";
export default {
  name: "Child",
...
```vue
<script>
  import { store, mutaions } from "../../store.oneself.js";
  export default {
    render(h) {
      return h("button", {
        // dom操作
        domProps: { 
          innerText: this.text + store.count  // 也可在render函数中使用。
        },
        on: {
          click: this.handleClick
        }
    ...
    methods: {
      handleClick() {
        this.$emit("clickEvent"); 
        mutaions.notifyCount(20); // 也可在render函数中使用。
      }
    }
  };
</script>
